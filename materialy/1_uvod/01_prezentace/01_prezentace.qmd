...
![](images/clipboard-1074189953.png)
...
Doporučuji absolutní cestu když už používat, tak pouze při nastavování `setwd()`. Sice lze teoreticky nahrávat soubory a provádět jiné akce uvnitř pracovního adresáře i pomocí absolutní cesty, ale j**akmile by se například složka ve které se pracuje kamkoliv posunula, tak by absolutní cesta přestala fungovat.**
...
2.  Vytvořte si v pokusnaSlozka sérii podsložek, a třeba až do třetí či páté úrovně (složka ve složce ve složce...) si nahrajte několik náhodných souborů. Vypište jejich seznam pomocí funkce `list.files()`.

## RProjekty

Co kdybychom chtěli, aby se nám všechny naše skripty a jejich výsledky ukládaly do a tahaly rovnou z **jednoho specifického adresáře** bez potřeby ho při každé práci na kódu nastavovat, který bychom navíc mohli jednoduše vzít a přesunout či ho někomu zazipovaný poslat?
...
V základním nastavení RProjekty **automaticky ukládají a následně nahrávají uložené proměnné zpět do globálního prostředí**. Není proto potřeba pči každém otevření RStudia celý skript projíždět znovu a všechno znovu nahrávat. Je ovšem možné si uložit vybranné proměnné i ručně.
