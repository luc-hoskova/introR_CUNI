...
```{r}
auta <- c(škoda = "octavia", škoda = "felicia", volkswagen = "beetle"); auta
```

::: callout-tip
Pojmenování jednotlivých částí můžeme použít i u ostatních datových struktur.
:::
...
-   ss
    ...
    ```{webr-r}
    #| context: output
    as.data.frame(cbind(1:4, c("MFF", "FHS", "1LF", "PřF"), c(FALSE, FALSE, TRUE, TRUE)))
    ```

-   xx

Co by se stalo kdybychom pouze použili cbind?
