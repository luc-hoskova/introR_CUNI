...
- **Konzole**: stisknutím Enter

- **Skript**:

  - jeden řádek: kliknu si do řádku a poté Ctrl + Enter (u Mac Option + Enter)

  - více řádků: myší si vyberu všechny řádky co chci spouštět a poté Ctrl + Enter (u Mac Option + Enter)

  - alternativa: je možné místo Ctrů + Enter spustit kód ve skriptu příkazem **`Run`**

    ![](images/clipboard-425036311.png)
...
Názvy proměnných jsou vždy bez uvozovek - nejedná se o text, ale o jméno specifického objektu.
...
::: callout-caution
## Název se nerovná charakter hodnotě

Uložené proměnné jsou vždy vypsané v globálním prostředí.

![](images/clipboard-2478750768.png)
:::
...
- začínat číslicí
  ...
  **Pozor na diakritiku!**
  ...
  ![](images/clipboard-902657775.png)

------------------------------------------------------------------------

**Jaké proměnné mám uložené v globálním prostředí?**

**Seznam proměnných**, které jsou v R uložené a se kterými můžete pracovat, **naleznete v poli Environment** v pravém horním rohu.

Můžete také ale využít funkce `objects()`. Ta do konzole vypíše názvy všech uložených hodnot.
...
Ulož do proměnné x číslo, do y text a do z boolean a následně je zobraz.
...
::: callout-important
## Hierarchie operátorů

Stejně jako násobení má přednost před sčítáním, tak i logické operátory mají svoji hierarchii.

**Operátor ! má vždy přednost před &, který má zase přednost před \| .**
:::

------------------------------------------------------------------------

![](images/transform-logical.png){fig-align="center" width="1045"}
...
Na dopravní hřiště mohou pouze děti ve věku 15 a více, mladší pouze s povolením rodičů. Může toto dítě na hřiště?
...
1.  Frantovi je 53 let. Chce si koupit jízdenku, u které je možnost slevy pro seniory od 65 let, pro děti a studenty až do 26 let včetně.\
    Ověř, zda má Franta na tuto slevu nárok.

2.  Studentstvo může získat stipendium za následujících dvou situací: mají průměr známek menší než 1.8 a zároveň absolvovali alespoň 80 % docházky, a nebo pokud mají potřebný průměr a reprezentují školu na soutěžích.\
...
3.  Uzavírají se známky na konci semestru. Zápočet dostanou pouze ti, kteří mají dosavadní hodnocení na lepším stupni než 5 a v posledním testu získali jiný počet než nula bodů. Pokud tyto podmínky nesplňují, tak zápočet mohou dostat i tehdy pokud vypracovali seminární práci navíc a v písemce měli alespoň 50 bodů.\
    Julie má dosavadní hodnocení sice na 5, ani písemka se jí nepovedla a měla pouze 17 bodů. Napsala ale velmi povedenou semestrální práci na téma kyberbezpečnosti ve Vietnamu.\
...
Má Saša nárok na stipendium?

::: fragment
Nejdříve si jednotlivé hodnoty uložíme.
...
::: fragment
Následně si je porovnáme s hodnotami podmínek.

```{r}
prumer < 1.8 & dochazka > 80 | prumer < 1.8 & reprezentace == TRUE
```
:::

::: fragment
Saša má právo na stupendium.
:::
...
::: fragment
Stejně jako u minulého cvičení, i zde si uložíme hodnoty do proměnných.
...
::: fragment
Hodnoty si dosadíme do výpočtu. Pozor na formulaci zadání!

```{r}
znamka < 5 & pocetBodu != 0 | semestralka == TRUE & pocetBodu >= 50
```
:::

::: fragment
Julie zápočet nedostane.
:::
...
Máme skript, pomocí kterého můžeme pracovat s daty, a který je dostatečně popsaný abychom ho mohli zaslat kolegům/sami pochopili, když se na něj podíváme později. **Skripty v R se ukládají jako soubor s koncovkou .R.**

Co když bychom si do R ale chtěli nahrát specifický dataset, který máme uložený v nějaké složce[^1]? Když si otevřeme nějaký skript, tak R vždy automaticky otevře a používá nejzákladnější složku v počítači (ve Windows `Dokumenty`).

[^1]: optimálně ne ve Stažené soubory ;)

![](images/clipboard-3376177032.png){fig-align="center"}
...
`File > New Project > bud' existující složka a nebo vytvoření nové`
:::
...
::: callout-important
## RProjekty mají implicitně nastavený adresář

Tím, že si vytvoříte RProjekt, tak si vytváříte novou složku/nastavujete složku, do které se vám budou ukládat veškeré vznikající skripty a RData soubory. **Nemusíte tak již adresář nastavovat pomocí `setwd()`!**
:::

## Nahrávání uložených proměnných

V základním nastavení RProjekty **umožňují ukládání a následné nahrávání proměnných, které jsme si vytvořili, zpět do globálního prostředí**. Není proto potřeba při každém otevření projektu v RStudiu celý skript projíždět znovu a všechno znovu nahrávat. Je ovšem možné si uložit vybranné proměnné i ručně.
